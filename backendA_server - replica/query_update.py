...
def search(topic):
    ...
    if len(posts) > 0:
    	...
    	return json.dumps({"result -from first replica":json_list})

    else:
        return jsonify({"result -from first replica":"make sure you enter a valid topic"})
...
def info(book_id):
    ...
    if post is not None:
        return jsonify(({"result -from first replica":{"item number": post['id'], "title": post['name'], "topic": post['topic'],
                     "price": post['cost'], "left in stock":  	post['number_of_items']}}))
    else:
        return jsonify({"result -from first replica":"make sure you enter a valid book id"})
